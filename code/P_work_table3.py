...
def get_detail(df):
    ...
    for x in df.index:
        ...
        detail=('参数信息\n'+'  整备质量:'+ get_not_nan(s,'整备质量(kg)_X_Max')+'\n'+
                '  B状态油耗,油耗/国标限值:'+ get_not_nan(s,'燃料消耗量(L/100km，B状态)_X_Min')+','+get_not_nan(s,'B状态油耗/国标限值')+'\n'+
                '  电耗,电耗优于门槛值:'+ get_not_nan(s,'工况条件下百公里耗电量(Y)(kWh/100km)_X_Min')+','+get_not_nan(s,'能耗/门槛')+'\n'+
                '  工况下纯电续航里程：'+ get_not_nan(s,'纯电动模式下续驶里程(km，工况法)_X_Max')+'\n'+
                '  电机总功率/电池容量：'+ mot_bat)
        list0.append(detail)
    return '\n'.join(list0)
...
def get_one_row(name0,name1,model,df):
    list0=[name0,name1,get_not_nan_name(df,'产品名称_M')]
